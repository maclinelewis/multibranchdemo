def send_results(workspace_fallout, type, send_img){
    bat """
        ...
        if $send_img EQU sendData ( GOTO SEND_IMG_DATA
        :SEND_IMG_DATA
        cd $workspace_fallout\\histogram_ref
        set ref_type= $type-ref-img
        for /r %%f in (*) do curl -i --form "myfile=@%%f" --form "build=%BUILD_NUMBER%" --form "branch_name=%BRANCH_NAME%" --form "type=%ref_type%" http://10.121.139.8:8000/send_data
        cd $workspace_fallout\\histogram_test
        set test_type= $type-test-img
        for /r %%f in (*.png) do curl -i --form "myfile=@%%f" --form "build=%BUILD_NUMBER%" --form "branch_name=%BRANCH_NAME%" --form "type=%test_type%" http://10.121.139.8:8000/send_data
        cd $workspace_fallout
        set img_type= $type-img
        for /r %%f in (*.png) do curl -i --form "myfile=@%%f" --form "build=%BUILD_NUMBER%" --form "branch_name=%BRANCH_NAME%" --form "type=%img_type%" http://10.121.139.8:8000/send_data
        )
        EXIT /b %ERRORLEVEL%   
    """
}
